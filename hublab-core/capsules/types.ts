...
// Alternative prop definition (object format)
export interface CapsulePropObject {
...
// Props can be array or object format
export type CapsuleProps = CapsuleProp[] | Record<string, CapsulePropObject>
...
export interface WebImplementation {
  ...
  components?: Record<string, string>  // Named components
...
export type CapsuleCategory =
  ...
  | 'screen'        // Full screen components
  | 'device'        // Device features (camera, biometrics)
  | 'utility'       // Utility components (QR codes, etc.)
